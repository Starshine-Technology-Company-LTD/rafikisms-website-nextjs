...
export const PricingCard = ({
  ...
  const cardClasses = `
    backdrop-blur-[14px] bg-gradient-to-br rounded-2xl shadow-xl flex-1 w-full sm:max-w-xs px-7 py-8 flex flex-col transition-all duration-300
    from-black/5 to-black/0 border border-black/10
    dark:from-white/10 dark:to-white/5 dark:border-white/10 dark:backdrop-brightness-[0.91]
    ${isPopular ? 'scale-105 relative ring-2 ring-teal-400/20 dark:from-white/20 dark:to-white/10 dark:border-teal-400/30 shadow-2xl' : ''}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans
    ${buttonVariant === 'primary' 
      ? 'bg-teal-600 hover:bg-teal-500 text-white shadow-sm' 
      : 'bg-teal-600/70 hover:bg-teal-500 text-white border border-teal-400/30'
    }
  `;
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-teal-600 text-white">
          Most Popular
        </div>
      )}
      <div className="mb-3">
      ...
      <div className="my-6 flex items-baseline gap-2">
        ...
        <span className="text-[14px] text-foreground/70 font-sans">TSH</span>
      </div>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-teal-500 w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
